...
// Our Story — ALWAYS show book modal on mobile (every visit)
(function storyBookModal(){
    if (!document.body.classList.contains('story')) return;

    // mobile/touch only; remove this guard if you want desktop too
    const isTouch = window.matchMedia('(hover: none)').matches;
    if (!isTouch) return;
    ...
    // Open on every page load (no localStorage gating)
    open();
